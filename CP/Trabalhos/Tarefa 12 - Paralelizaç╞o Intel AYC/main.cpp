...
/**
Os tempos analisados foram todos em relação ao "Work hard time"

Sequencial
Work hard time = 5.920293
* 
Paralelo com schedule padrao (static,1)
Work hard time = 3.836209
*
Paralelo com schedule (static,7000)
Work hard time = 3.405083
 
 PRECISA DE UM CRITICAL NO METODO PRINCIPAL (TURMA TODA TIROU 0)
...
void compute_path(vector<Flight>& flights, string to, vector<Travel>& travels, unsigned long t_min, unsigned long t_max, Parameters parameters){
  ...
  while(travels.size() > 0){
    ...
    if(current_city.to == to){
	  ...
	  #pragma omp for private(travel,current_city) schedule(static,7000)
      for(unsigned int i=0; i<flights.size(); i++){
		Flight flight = flights[i];
		if(flight.from == current_city.to && flight.take_off_time >= t_min &&
	    flight.land_time <= t_max && (flight.take_off_time > current_city.land_time) &&
